...
const createDivRenderSpec = (divId: string, spec: any) => {
  // Create a container div if it doesn't exist
  let container: HTMLElement | null = document.getElementById(`#${divId}`);
  if (!container) {
...
function readFileSyncNoLibs(path: string): any {
...
const renderMyStuff = async () => {
  const seattleWeatherJson = await readFileSyncNoLibs(
    "data/seattle-weather.csv",
  );

  const weeks: Record<number, any> = {};
  let curr_week = 0;
  seattleWeatherJson.map((d: any, ind: number) => {
    if (ind % 7 === 0) {
      curr_week = ind / 7;
      weeks[curr_week] = {};
    }
